...
在 Windows 下，我们要把路径格式改成 Windows 的本地路径（例如把配置和电影放在 D:\Jellyfin 盘下）。

在 c 盘创建一个名为 Jellyfin 的文件夹，里面再建三个子文件夹：config、cache、movies。

在 c:\Jellyfin 目录下创建一个 docker-compose.yml 文件，写入以下内容：
...
打开 Windows PowerShell，切换到该目录并启动：
...
Jellyfin 电视直播必须绑定 EPG 节目单才能激活前端频道。在宿主机 C:\Jellyfin\media 目录下新建两个本地文件：
...
[http://服务器地址:8888/hls/index.m3u8](http://服务器地址:8888/hls/index.m3u8)
3. Jellyfin 后台电视直播对接
进入 Jellyfin 后台 ➔ 电视直播。

调谐器设备： 添加，类型选择 M3U，路径填写本地容器映射路径或直接填写绝对路径：C:\Jellyfin\media\tv.m3u。

电视节目指南数据提供商： 添加，类型选择 XMLTV，路径填写：C:\Jellyfin\media\live.xml。

进入 计划任务 ➔ 执行 “刷新电视节目指南”，即可激活前端频道。
...
原理： 限制条件永远无法触发，系统对任何终端均“只播不记”，关闭网页即蒸发进度，下次点开永远从 00:00 从头播放。

2. 净化主页 UI
进入该公共账号的 首页 设置标签页，将 主屏幕模块 中的“继续观看”彻底修改为 “无”，使多台设备并发登录时的主页更加干净整洁。
